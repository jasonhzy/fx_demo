...
class View{
	...
	*/ 
    ...
    function img($name = null){
        ...
        $fn = APP_PATH.DS.$this->Controller->__module.DS.'images'.DS.$name;
        if(file_exists($fn)){
            return $this->__siteurl.(SYS_PATH == dirname(APP_PATH) ? '' : basename(dirname(APP_PATH)).'/').basename(APP_PATH).'/'.$this->Controller->__module."/images/{$name}";
        }else{
            $imgurl = $this->__siteurl.(SYS_PATH == dirname(APP_PATH) ? '' : basename(dirname(APP_PATH)).'/')."images/{$name}";
			if( !file_exists( THIS_PATH.basename(dirname(APP_PATH))."/images/{$name}" ) ){
				$imgurl = dirname($this->__siteurl).'/'.basename(dirname(APP_PATH))."/images/{$name}";
			}
			return $imgurl;
        }*/
		if(isset($this->__imgurl) && $this->__imgurl){
     ...
     function js($name = null){ 
        if(is_string($name)){   
			if(file_exists(dirname(APP_PATH).DS.$name)&&is_file(dirname(APP_PATH).DS.$name)){
        ...
        if(is_array($name)){
            foreach($name as $var){
				if(file_exists(dirname(APP_PATH).DS.$var)&&is_file(dirname(APP_PATH).DS.$var)){
