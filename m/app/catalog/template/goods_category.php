...
<?php $imgs = array(); if(!empty($rt['categoodslist']))foreach($rt['categoodslist'] as $k=>$row){ $imgs[] = $row['goods_img'];?>
	<li style="width:50%; float:left;">
		...
		<a style="background:#fff; padding:5px; display:block;" href="<?php echo ADMIN_URL.($row['is_jifen']=='1'?'exchange':'product').'.php?id='.$row['goods_id'];?>">
			...
			<p style="line-height:26px; height:26px; overflow:hidden;"><b style="margin-top:5px;line-height:22px;color:#FE0000; font-size:14px; float:left; margin-right:5px; padding-left:3px; padding-right:5px;">￥<?php echo str_replace('.00','',$row['pifa_price']);?></b></p>
