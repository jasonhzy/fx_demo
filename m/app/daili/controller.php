...
class DailiController extends Controller{
	...
	function gonggao(){
		...
		$this->title("代理资讯专区".' - '.$GLOBALS['LANG']['site_name']);
		$this->is_daili();
		...
		if(!defined(NAVNAME)) define('NAVNAME', "代理资讯专区");
		$this->set('rt',$rt);
		$this->template('v2_gonggao');
	}
	...
	function my_is_daili(){
		...
		if(!defined(NAVNAME)) define('NAVNAME', "成为分销");
		$this->set('rt',$rt);
		$this->template('v2_myuser');
	}
	...
	function monrydeial(){
		...
		$rt['pages'] = Import::basic()->getpage($tt,$list,$page,'?page=',true);
		$sql = "SELECT tb1.*,tb2.nickname FROM `{$this->App->prefix()}user_money_change` AS tb1 LEFT JOIN `{$this->App->prefix()}user` AS tb2 ON tb1.buyuid = tb2.user_id WHERE tb1.uid='$uid' ORDER BY tb1.time DESC LIMIT $start,$list";
		$rt['lists'] = $this->App->find($sql); //商品列表
		$rt['page'] = $page;
		
		if(!defined(NAVNAME)) define('NAVNAME', "收入明细");
		$this->set('rt',$rt);
		$this->template('v2_monrydeial');
	}	
	...
	function ajax_postmoney($data=array()){
		...
		$money = $data['money'];
/*		if($money < 50){
			echo "暂时不能为你服务，先赚取50以上佣金再来吧！";exit;
		}*/
		//检查密码
		$pass = md5(trim($pass));
		$sql = "SELECT id FROM `{$this->App->prefix()}user_bank` WHERE pass = '$pass' AND uid='$uid' LIMIT 1";
		$id = $this->App->findvar($sql);
		if($id > 0){
			//检查资金
			$sql = "SELECT mymoney FROM `{$this->App->prefix()}user` WHERE user_id='$uid' LIMIT 1";
			...
			if(intval($mymoney) < intval($money)){
				echo "资金不足，你不能提款！"; exit;
			}
			$money = number_format($money, 2, '.', '');
			if(intval($money) < 50){
				echo "提款金额最低50元起！"; exit;
			}
			$dd = array();
			$dd['uid'] = $uid;
			$dd['money'] = $money;
	...
	function myuserinfo($data=array()){
		...
		$this->set('rt',$rt);
		$this->template('v2_myuserinfo');
	}
	
	//我的客户
	function myuser(){
		$uid = $this->Session->read('User.uid');
		if(!defined(NAVNAME)) define('NAVNAME', "我的客户");
