		...
		if(isset($_POST)&&!empty($_POST)){
			...
			if($this->App->insert('goods_order_info',$orderdata)){ //订单成功后
				...
				if($this->App->insert('user_point_change',array('time'=>mktime(),'changedesc'=>"用积分兑换商品",'uid'=>$uid,'points'=>-intval($jifen_onetotal)))){
					...
					$sql = "UPDATE `{$this->App->prefix()}user` SET `points_ucount` = `points_ucount`+$jifen_onetotal WHERE user_id = '$uid'";
					$this->App->query($sql);
				}
				...
				$this->jump(ADMIN_URL.'excart.php',0,'你的订单没有提交成功，我们是尽快处理出现问题！'); exit;
			}
