		...
		$rt = $this->Cache->read(3600);
	 	if(is_null($rt)) {
			...
			if(!empty($rt['cat']))foreach($rt['cat'] as $row){
				...
				$sql = "SELECT goods_id,goods_name,goods_thumb,goods_img,pifa_price,shop_price FROM `{$this->App->prefix()}goods` WHERE cat_id IN(".implode(',',$sub_cids).") AND is_on_sale='1' AND is_jifen = '0' AND is_delete = '0' AND (is_best ='1' OR is_new='1' OR is_hot='1') ORDER BY sort_order ASC LIMIT 6";
