...
/*		$pu = $this->App->findrow("SELECT user_id,daili_uid,parent_uid,parent_uid2,parent_uid3,goods_amount,order_amount,order_sn,pay_status,order_id FROM `{$this->App->prefix()}goods_order_info` WHERE order_sn='$order_sn' LIMIT 1");
		...
		if($tt == 'true' && $status=='1' && !empty($order_sn)){
			...
			//计算资金，便于下面返佣
			//计算每个产品的佣金
			$sql = "SELECT takemoney1,takemoney2,takemoney3 FROM `{$this->App->prefix()}goods_order` WHERE order_id='$order_id'";
...
/*							$p4_uid = $this->return_daili_uid($p3_uid);
   ...
   function update_user_tree($puid = 0,$ppuid=0)
	...
	function get_openid_AND_pay_info(){
		...
		$sql = "SELECT order_sn,order_amount FROM `{$this->App->prefix()}goods_order_info` WHERE pay_status = '0' AND order_sn='$order_sn' LIMIT 1";
		$rt = $this->App->findrow($sql);
		if(empty($rt)){
			$this->jump(ADMIN_URL,0,'非法支付提交！'); exit;
		}
	...
	*/
	function confirm(){
		...
		if(isset($_POST)&&!empty($_POST)){
			...
			$pr = $this->App->findrow("SELECT parent_uid,daili_uid FROM `{$this->App->prefix()}user_tuijian` WHERE uid='$uid' LIMIT 1");
			//$sql = "SELECT tb1.parent_uid AS p1,tb1.daili_uid AS d1,tb2.parent_uid AS p2,tb2.daili_uid AS d2,tb3.parent_uid AS p3,tb3.daili_uid AS d3 FROM `{$this->App->prefix()}user_tuijian` AS tb1";
			//$sql .=" LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.uid = tb1.parent_uid";
			//$sql .=" LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb3 ON tb3.uid = tb2.parent_uid";
			//$sql .=" WHERE tb1.uid='$uid' LIMIT 1";
			//$pr = $this->App->findrow($sql);
			$parent_uid = isset($pr['parent_uid']) ? $pr['parent_uid'] : 0;
			$daili_uid = isset($pr['daili_uid']) ? $pr['daili_uid'] : 0;
			if(!($daili_uid > 0)){
				if($parent_uid >0){
					$rd = $this->App->findvar("SELECT user_rank FROM `{$this->App->prefix()}user` WHERE user_id='$parent_uid' LIMIT 1");
					if($rd!='1'){
						$daili_uid = $parent_uid;
					}
				}
			}
			//$orderdata['parent_uid'] = $parent_uid >0 ? $parent_uid : '0'; //上一级
			$orderdata['parent_uid'] = $daili_uid >0 ? $daili_uid : '0'; //一级代理
			//$orderdata['daili_uid'] = $orderdata['parent_uid'];
			//查找二级、三级代理
			if($daili_uid > 0){
...
/*					$gid = $rows['goods_id'];
	...
	function ajax_change_price($data=array()){
		...
		unset($cartlist);
		$result = array('error' => 0, 'message' => '1','prices'=>$prices,'thisprice'=>$thisprice);
		die($json->encode($result));
	}
