...
.addgallery{ padding-left:14px;padding-right:14px; background:url(<?php echo $this->img('++.png');?>) center center no-repeat}
.removegallery{ padding-left:14px;padding-right:14px; background:url(<?php echo $this->img('--.png');?>) center center no-repeat}
</style>
		...
		<table border="0" cellpadding="0" cellspacing="0" style="width:100%;border-radius:5px; border:1px solid #ededed; margin-top:10px;">
			<tr>
				...
				<?php if(!empty($row['spec'])){
				 ...
				 <p style=" padding-left:10px;font-size:12px;line-height:24px;" class="raturnprice raturnprice<?php echo $k;?>">会员价:<font color="#FF0000">￥<?php echo $row['pifa_price'];?></font>&nbsp;&nbsp;惊喜价:<font color="#FF0000" class="gprice<?php echo $row['goods_id'];?>">￥<?php echo $row['price'];?></font></p>
					...
					if(!empty($useradd[$k]))foreach($useradd[$k] as $kk=>$ress){
					 ?>
						...
						<tr>
							...
							<input alt="<?php echo $k;?>" id="<?php echo $kk;?>" type="text" value="<?php echo $ress['address'];?>" name="address"  class="pw postdata" style="width:100%;"/> 
							</td>
						</tr>
						</table>
						<?php } ?>
...
$('.postdata').blur(function(){
	...
	id = $(this).attr('alt');
	kk = $(this).attr('id');
	$.post('<?php echo $thisurl;?>',{action:'ajax_change_carval',type:alt,val:val,gid:id,kk:kk},function(prices){
