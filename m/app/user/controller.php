...
class UserController extends Controller{
	...
	function shoplist($data=array()){
		$uid = $this->Session->read('User.uid');
		$sql = "SELECT tb1.author,tb1.article_title,tb1.article_img,tb1.article_id FROM `{$this->App->prefix()}article` AS tb1 LEFT JOIN `{$this->App->prefix()}article_cate` AS tb2 ON tb2.cat_id = tb1.cat_id WHERE tb2.type='new' ORDER BY tb1.vieworder ASC,tb1.article_id DESC";
	...
	function myerweima(){
		...
		$uid = $this->Session->read('User.uid');
		$filename = $uid.'.png';
		$f = SYS_PATH_PHOTOS.'qcody'.DS.$uid.DS.$filename;
		$thisurl = Import::basic()->thisurl();
		if(!(is_file($f)) || !file_exists($f) || (mktime() - filemtime($f) > 7000)){
			$this->action('common','mark_phpqrcode',$filename,$thisurl);
		}
		...
		$this->set('qcodeimg',SITE_URL.'photos/qcody/'.$uid.'/'.$uid.'.png');
		$this->template('myerweima');
	}
	...
	function index(){ 
		...
		$this->action('common','checkjump');
		//if(!($uid>0) || $rank !='10'){ $this->jump(ADMIN_URL.'user.php?act=login',0,'请先登录！'); exit;}
/*		$sql = "SELECT id,share_uid,parent_uid FROM `{$this->App->prefix()}user_tuijian`";
		$rts = $this->App->find($sql);
		foreach($rts as $row){
			if(!($row['parent_uid'] > 0)){
				$this->App->update('user_tuijian',array('parent_uid'=>$row['share_uid']),'id',$row['id']);
			}
		}*/
...
/*		
		...
		$this->set('rt',$rt);
		$mb = $GLOBALS['LANG']['mubanid'] > 0 ? '_'.$GLOBALS['LANG']['mubanid'] : '';
		$this->set('mubanid',$GLOBALS['LANG']['mubanid']);
		$this->template('index'.$mb);
	}
	...
	function apply(){
		...
		//if(!($uid>0)){ $this->jump(ADMIN_URL.'user.php?act=login',0,'请先登录！'); exit;} //
		...
		if($rt['userinfo']['user_rank']!='1' && $rt['userinfo']['is_salesmen']=='2'){ //已经申请成功的代理要跳转
			$this->jump(ADMIN_URL.'user.php?act=dailicenter');exit;
		}
	...
	function dailicenter(){
		...
		/*$rrL = $this->action('common','_get_appid_appsecret');
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if(!empty($key)){
				...
				if( $bonus_id > 0){
					...
					$this->jump(ADMIN_URL.'user.php?act=youhuijuan',0,'你已成功得到优惠劵！');exit;
				}else{
	...
	function __order_list($w_rt=array(),$page=1,$list=5){
		...
		if(!$page) $page=1;
		$start = ($page-1)*$list;
		$sql = "SELECT distinct tb1.order_id, tb1.order_sn, tb1.order_status, tb1.shipping_status,tb1.shipping_name ,tb1.pay_name, tb1.pay_status, tb1.add_time,tb1.consignee, (tb1.order_amount + tb1.shipping_fee) AS total_fee FROM `{$this->App->prefix()}goods_order_info` AS tb1";
